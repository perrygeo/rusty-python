...
from rusty_python import sum_as_string, sum_as_string_py

# Numpy interop
import numpy as np
from rusty_python import mult_arrays
...
def test_numpy_mult_shape():
    ...
    a = np.array([1, 2, 3, 4], dtype=np.float64)  # matches f64 in Rust
    b = np.array([5, 6, 7, 8], dtype=np.float64)
    d = mult_arrays(a.reshape(2, 2), b.reshape(2, 2))
    assert d.sum() == 70
